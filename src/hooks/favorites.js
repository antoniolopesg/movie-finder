...
export const FavoritesProvider = ({children}) => {
    ...
    async movie => {
      ...
      if (movie.poster) {
        try {
          ...
          const downloadImg = FileSystem.createDownloadResumable(
            movieToFav.poster,
            fileUri
          );
          ...
          Alert.alert(
            ...
            'Sorry, some problem happened when adding the movie to favorites'
          );
          return;
        }
      }
  ...
  const isFavorite = useCallback(
    id => favorites.findIndex(item => item.id === id) >= 0,
    [favorites]
  );
  ...
  return (
    <FavoritesContext.Provider
      value={{favorites, toggleFavorite, isFavorite, findFavorite}}>
      {children}
    </FavoritesContext.Provider>
  );
};
